...
# return the name of the xyz_column
def preprocess_atomxyz_columns(path:str,
                               atom_xyz_columns:Union[str, List[str]] = None):

    if atom_xyz_columns is None:
        if os.path.isfile(path):
            columns = get_header(path)
            atom_xyz_columns = columns[2:3]
        else:
            atom_xyz_columns = [None] * 2
    else:
        if isinstance(atom_xyz_columns, str):
            atom_xyz_columns = [atom_xyz_columns]
        if os.path.isfile(path):
            columns = get_header(path)
            if any([atom_xyz not in columns for atom_xyz in atom_xyz_columns]):
                raise ValueError('Provided smiles_columns do not match the header of data file.')

    return atom_xyz_columns

# return the name of the atommap column 
def preprocess_atommap_columns(path:str,
                               atom_map_columns:Union[str, List[str]] = None):
    """
    Preprocesses the :code:`smiles_columns` variable to ensure that it is a list of column
    headings corresponding to the columns in the data file holding SMILES. Assumes file has a header.
    :param path: Path to a CSV file.
    :param atom_map_columns: The names of the columns containing Atom_Map.
                           By default, uses the second column

    :return: The preprocessed version of :code:`atom_map_columns` which is guaranteed to be a list.
    """

    if atom_map_columns is None:
        if os.path.isfile(path):
            columns = get_header(path)
            atom_map_columns = columns[1:2]
        else:
            atom_map_columns = [None] * 1
    else:
        if isinstance(atom_map_columns, str):
            atom_map_columns = [atom_map_columns]
        if os.path.isfile(path):
            columns = get_header(path)
            if any([atom_map not in columns for atom_map in atom_map_columns]):
                raise ValueError('Provided smiles_columns do not match the header of data file.')

    return atom_map_columns
...
def get_data(path: str,
    ...
    if args is not None:
        ...
        atom_map_columns = atom_map_columns if atom_map_columns is not None else args.atom_map_columns
        atom_xyz_columns = atom_xyz_columns if atom_xyz_columns is not None else args.atom_xyz_columns
    ...
    # 确保得到smiles、atommap、xyz所在的列名
    if isinstance(smiles_columns, str) or smiles_columns is None:
        smiles_columns = preprocess_smiles_columns(path=path, smiles_columns=smiles_columns)
    if isinstance(atom_map_columns, str) or atom_map_columns is None:
        atom_map_columns = preprocess_atommap_columns(path=path, atom_map_columns=atom_map_columns)
    if isinstance(atom_xyz_columns, str) or atom_xyz_columns is None:
        atom_xyz_columns = preprocess_atomxyz_columns(path=path, atom_xyz_columns=atom_xyz_columns)
    # 
    ...
    with open(path) as f:
        ...
        if any([c not in fieldnames for c in atom_map_columns]):
            raise ValueError(f'Data file did not contain all provided atommap columns: {atom_map_columns}. Data file field names are: {atom_map_columns}')
        if any([c not in fieldnames for c in atom_xyz_columns]):
            raise ValueError(f'Data file did not contain all provided atomxyz columns: {atom_xyz_columns}. Data file field names are: {atom_xyz_columns}')
        ...
        for i, row in enumerate(tqdm(reader)):
            smiles = [row[c] for c in smiles_columns]
            atom_map = [row[c] for c in atom_map_columns]
            atom_map = ast.literal_eval(atom_map[0])   # list:[]
            atom_xyz =  [row[c] for c in atom_xyz_columns]
            atom_xyz =  ast.literal_eval(atom_xyz[0])  # list:[]
            ...
            all_atom_map.append(atom_map) #[[],[],[]...],每一个一维list代表一个分子中的atom_map关系
            all_atom_xyz.append(atom_xyz)# [[],[],[]...]，每一个一维list代表一个分子中的所有原子的坐标
        ...
        data = MoleculeDataset([
            ...
            ) for i, (smiles, targets,atom_map,atom_xyz) in tqdm(enumerate(zip(all_smiles, all_targets,all_atom_map,all_atom_xyz)),
