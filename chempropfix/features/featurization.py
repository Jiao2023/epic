...
class MolGraph :
    ...
    * :code:`overwrite_default_bond_features`: A boolean to overwrite default bond descriptors.
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        """
        self.is_mol = is_mol(mol) # true：此时分子类型为pybel.Molecule
        ...
        if type(mol) == str:
            ...
            # 暂时缺少： make_polymer_mol make_normal_mol
                ...
                if mtype == 0:
                    fragment_weights = [0.5,0.5] #[0.5,0.5]
                elif mtype == 1:
                    fragment_weights = [0.75,0.25] #[0.75,0.25]
                elif mtype == 2:
                    fragment_weights = [0.25,0.75] #[0.25,0.75]
                elif mtype == 4: # acid_glycol data
                    fragment_weights=mol.split("|")[1:]
                    fragment_weights = [float(i)/100 for i in fragment_weights]
                    mol = mol.split("|")[0]
                # mol, mol_lens = make_normal_mol(mol, self.is_explicit_h, self.is_adding_hs,fragment_weights)
...
# 师兄代码
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    ...
                    # 如果bond feature需要包含atom就不要注释下面两行
                    if is_wDMPNN:
                        self.f_bonds.append(self.f_atoms[a1] + f_bond)
                        self.f_bonds.append(self.f_atoms[a2] + f_bond)
                    # 如果bond feature需要包含atom就注释下面两行
                    # ------- 师兄代码 ----------
                    else:
                        self.f_bonds.append(f_bond)
                        self.f_bonds.append(f_bond)
                    #self.mol_lens = mol_lens
                    # ---------------------------
...
# --------------------------
        ...
        # edge_index=torch.tensor(edge_index),),max_len=250) #it should be args.max_atom_len but I feel tired to do it 
# 69
    ...
    * :code:`a2a`: (Optional): A mapping from an atom index to neighboring atom indices.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        for mol_graph in mol_graphs:
            ...
            f_maps.extend(mol_graph.f_map)
            #f_xyzs.append(mol_graph.f_xyz)
            f_xyz = np.array(mol_graph.f_xyz).reshape(-1,3)
            for line in range(f_xyz.shape[0]):
                f_xyzs.append(f_xyz[line])

            #w_atoms.extend(mol_graph.w_atoms)
            #w_bonds.extend(mol_graph.w_bonds)
            # if 'mol_lens' in dir(mol_graph):
            #mol_lens.append(mol_graph.mol_lens)
        ...
        self.f_map = torch.tensor(f_maps, dtype=torch.long).squeeze()
        self.f_xyz = torch.tensor(f_xyzs, dtype=torch.float).squeeze()
        #self.mol_lens = torch.LongTensor(mol_lens)
        # self.mol_lens = torch.LongTensor([])
    ...
    def get_components(self, atom_messages: bool = False):
        ...
        * :code:`b_scope`
        ...
        # }
            ...
            # self.mol_lens,self.w_atoms, self.w_bonds,self.degree_of_polym,self.distances, self.node_paths, self.edge_paths
        return self.f_atoms, f_bonds,self.a2b, self.b2a, self.b2revb, \
               self.a_scope, self.b_scope,self.f_map, self.f_xyz
