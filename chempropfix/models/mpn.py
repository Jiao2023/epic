...
# 设置日志格式
logging.basicConfig(filename='/home/jiaopanyu/example.log', level=logging.DEBUG,
...
def reorder_f_xyz(f_xyz,f_map,a_scope):
    total_f_atoms = []
    for item in a_scope:
        atom_idx = item[0]
        n_atom = item[1]
        f_map_list = f_map[atom_idx:atom_idx+n_atom]
        atom_xyz_tensor = f_xyz[atom_idx:atom_idx+n_atom,:]
        reorder_f_atoms = torch.stack([atom_xyz_tensor[i] for i in f_map_list])
        total_f_atoms.append(reorder_f_atoms)

    f_xyz = torch.cat(total_f_atoms, dim=0) 
    return f_xyz
class mol_graph2data(nn.Module):
    ...
    def forward(self,
        ...
        f_xyz = reorder_f_xyz(f_xyz,f_map,a_scope)
        ...
        return f_atoms[1:,:].to(self.args.device),\
               ...
               b2revb_individual, \
               batch, ptr,f_xyz.to(self.args.device)
...
class pyG_helper(nn.Module):
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int):
        ...
        if args.encoder_type in ["gcn","gcn_attn","gcn_pe"]:
            ...
            self.mpn = POLYGIN(in_channels=atom_fdim,hidden_channels=args.hidden_size,num_layers=args.depth,dropout=args.gnn_dropout,activation=get_activation_function(args.activation),edge_hidden=bond_fdim+1,node_hidden=args.hidden_size)
        #self.with_attn = args.with_attn
        #if self.with_attn:
        #    self.attention_encoder = AttnEncoderXL(args,bond_fdim)
    ...
    def forward(self,
        ...
        x, edge_index, edge_attr, b2revb_individual, batch, ptr,f_xyz = self.mol_graph2data_layer(mol_graph) 
